...
export function renderShipmentNotification(input: {
  ...
  const bodyHtml = tracking.length === 0 ? "" : `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;">
  <tr>
  ...
  ${tracking.map(({ number, url }) => [
    ...
    <td style="padding:10px 0;border-bottom:1px solid #d9d4c6;font-size:14px;line-height:20px;">${escapeHtml(number)}</td>
  </tr>` : "",
    url ? `<tr>
    <td style="padding:10px 12px 10px 0;border-bottom:1px solid #d9d4c6;font-size:14px;line-height:20px;">Tracking link</td>
    <td style="padding:10px 0;border-bottom:1px solid #d9d4c6;font-size:14px;line-height:20px;"><a href="${escapeHtml(url)}">${escapeHtml(url)}</a></td>
  </tr>` : "",
  ].join("\n")).join("\n")}
</table>`;
