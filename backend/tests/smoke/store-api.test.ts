...
import { createMedusaStoreClient } from "../../../storefront/src/lib/medusa-client.js";
import {
  STORE_CART_FIELDS,
  addStoreLine,
  ...
  createStoreCart,
  retrieveStoreCart,
} from "../../../storefront/src/lib/store-cart.js";
...
/**
 * A basket, built the way the storefront builds one, and read back through the
 * storefront's own parser.
 *
 ...
 * Nothing has changed." The cause was not the cart at all — every Store call
 * succeeded, and a `curl` reproduction of the same three requests returned 200
 ...
 * `items[].total` *absent* — not zero, missing — while the cart-level
 * `item_total` is computed and correct. `cartLinesFromStore` reads each line's
 * `total`, so it threw on every add, restore, quantity change and removal.
 *
 * ## Why nothing caught it
 *
 * Every layer was tested against something that agreed with it. The mock cart
 * layer satisfies the parser; the Playwright fixture hand-writes `total` on
 * every line it returns, unconditionally, so the browser suite drove a
 * synthetic Medusa more generous than the real one; and this suite asked the
 * catalogue a question but never built a cart. 2,994 unit tests and 47 browser
 * tests passed with the buy button broken.
 *
 * So the assertion below is deliberately *not* "the response has a total". It
 * runs the real parser over the real response, because the question worth
 * asking is whether the two agree — and the parser is the thing that will
 * change next.
 */
describe("the basket the storefront builds", () => {
  async function cart(
    path: string,
    init?: { readonly method?: string; readonly body?: string },
  ): Promise<{ status: number; body: unknown; text: string }> {
    return await json(path, { ...init, headers: { "x-publishable-api-key": publishableKey } });
  }

  async function newCartId(): Promise<string> {
    const created = await cart("/store/carts", {
      method: "POST",
      body: JSON.stringify({ region_id: regionId }),
    });
    expect(created.status, created.text).toBe(200);
    return String(record(record(created.body, "cart response")["cart"], "cart")["id"]);
  }

  /** The one variant, as `addStoreCatalogueLine` finds it. */
  async function variantId(): Promise<string> {
    const products = await cart(`/store/products?limit=1&fields=id,variants.*&region_id=${regionId}`);
    expect(products.status, products.text).toBe(200);
    const list = sequence(record(products.body, "products response")["products"], "products");
    const variants = sequence(record(list[0], "product")["variants"], "variants");
    return String(record(variants[0], "variant")["id"]);
  }

  it("carries per-line totals when the request asks for them, and not otherwise", async () => {
    const id = await newCartId();
    const variant = await variantId();
    const body = JSON.stringify({ variant_id: variant, quantity: 1 });

    const bare = await cart(`/store/carts/${id}/line-items`, { method: "POST", body });
    expect(bare.status, bare.text).toBe(200);
    const bareItems = sequence(record(record(bare.body, "response")["cart"], "cart")["items"], "items");
    expect(
      record(bareItems[0], "line")["total"],
      "Medusa began returning per-line totals by default; STORE_CART_FIELDS and this test can be simplified",
    ).toBeUndefined();

    const asked = await cart(
      `/store/carts/${id}/line-items?fields=${encodeURIComponent(STORE_CART_FIELDS)}`,
      { method: "POST", body },
    );
    expect(asked.status, asked.text).toBe(200);
    const askedItems = sequence(record(record(asked.body, "response")["cart"], "cart")["items"], "items");
    expect(record(askedItems[0], "line")["total"], "the fields were asked for and not answered").toBeTypeOf("number");
  });

  /**
   * The remaining two drive `src/lib/store-cart.ts`'s **own functions** against
   * this server, rather than building the requests here.
   *
   * That distinction is the difference between a test that pins the fix and a
   * test that would have caught the defect. A test that assembles its own URL
   * with `STORE_CART_FIELDS` in it passes whether or not the storefront
   * remembers to send them; only calling the shipped function proves the
   * shipped function works.
   */
  function storefrontClient(): StoreClient {
    return new Medusa({ baseUrl: backendUrl, publishableKey }) as unknown as StoreClient;
  }

  it("adds a line through the storefront's own code, for a destination that is taxed", async () => {
    const sdk = storefrontClient();
    const id = await createStoreCart(sdk, regionId);
    await applyDestinationToCart(sdk, id, "ee");

    const lines = await addStoreLine(sdk, id, await variantId());

    expect(lines, "the basket must hold the one line that was added").toHaveLength(1);
    expect(lines[0]!.quantity).toBe(1);
    expect(lines[0]!.variantId, "no variant reached the line; availability is being guessed").toBeTypeOf("string");
    /*
     * Estonia is inside the VAT union, so the line the buyer is charged for is
     * the gross figure. A parser reading `unit_price` would report the net one
     * and the basket would understate the price by the VAT.
     */
    expect(lines[0]!.unitAmount).toBe(GROSS_MINOR);
  });

  it("restores a basket through the storefront's own code — the path the buy button lands on", async () => {
    const sdk = storefrontClient();
    const id = await createStoreCart(sdk, regionId);
    await applyDestinationToCart(sdk, id, "ee");
    await addStoreLine(sdk, id, await variantId());

    // What `/cart` does on load with a remembered cart id.
    expect(cartLinesFromStore(await retrieveStoreCart(sdk, id))).toHaveLength(1);
  });
});
