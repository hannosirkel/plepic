...
| The web address of the Consumer Disputes Committee | nothing — **an enhancement, not a blocker** | **Not supplied, and not required.** The operator, with the qualified reviewer's manual verification on 2026-08-10, ruled that naming the out-of-court body without an access method satisfies Article 6(1)(t) CRD, and `/legal/terms` names the Consumer Disputes Committee in its own prose. `content/` may carry no host, so the address arrives as `EXTERNAL_URL_CONSUMER_DISPUTES_COMMITTEE` — and until a deployment sets it, the page renders one link fewer and **nothing else changes**: no gap marker, no incompleteness notice. Supplying it improves the page; withholding approval for it does not |
| An Estonian-language version of the legal set | all five legal pages | **Recorded as a deviation, not acted on here.** Language Act (keeleseadus) § 16 and the Consumer Protection Act entitle a consumer in Estonia to consumer information in Estonian, and an Estonian-registered shop shipping to Estonia with English-only legal pages is exposed with the TTJA. The second read required this to be *"either satisfied or recorded as an accepted deviation"*; this is the record. **The exposure still stands, and it is still the operator's to accept or close: there is no Estonian on this site.** What has changed since that record was written is only the shape underneath it. The content model now carries a locale dimension at exactly one locale (`en`): `LOCALES` and `LOCALE_DEFINITIONS` in `routes.ts`, `LocalizedContent<T>` in `schema.ts`, `legalPagesByLocale` in `legal/index.ts` and `pagesByLocale` in `index.ts`, with the served document's language, the `hreflang` alternates, the canonicals, the sitemap and a prefixed route subtree all computed from it. Not one word of the five notices changed to get there — the English edition is registered by a key written around those files, not by a field added inside them. Translation remains the separate scheduled unit (`t2-estonian-locale`) with its own operator-decided provenance: the agent drafts, a language pass reviews, and the qualified reader accepts |
